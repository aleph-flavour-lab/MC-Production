#!/usr/bin/env python3
"""
Run RWedm4hep_exec in parallel on all *.fort64 files.
Output .root files are saved to OUTPUT_DIR.

Usage:
  python run_edm4hep.py              # process all files, 8 workers
  python run_edm4hep.py --dry-run
  python run_edm4hep.py --workers 4
...
# Configuration
# ---------------------------------------------------------------------------
INPUT_DIR  = Path("/eos/user/h/hfatehi/ALEPH-DATA/1992")
OUTPUT_DIR = Path("/eos/user/h/hfatehi/ALEPH-DATA/1992/EDM4HEP")
...
def process_file(fort64_path: Path) -> tuple[str, bool, str]:
    stem     = fort64_path.name.replace(".fort64", "")  # e.g. Y15222.24
    out_file = OUTPUT_DIR / f"{stem}.root"

    if out_file.exists():
        return (fort64_path.name, True, "SKIP (already exists)")

    cmd = [str(RW_BIN), "--input", str(fort64_path), "--output", str(out_file)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run RWedm4hep in parallel on all fort64 files."
    )
    ...
    fort64_files = sorted(INPUT_DIR.glob("*.fort64"))
    ...
    # Filename format: Y15222.24.fort64 -> run number is first numeric group (15222)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if ok:
                success += 1
                print(f"[{done}/{total}] {basename:<30} {msg}")
            else:
                failed.append(basename)
                print(f"[{done}/{total}] FAILED {basename:<25} {msg}")
