...
def copy_to_outdir(src, outdir, log):
    """Copy src file to outdir, return destination path."""
    import shutil
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    dst = os.path.join(outdir, os.path.basename(src))
    shutil.copy2(src, dst)
    log('Copied final output to %s' % dst)
    return dst
...
def run_job(loseed, inseed, ecms, flavor, date, dbase, root_workdir, dry_run, last_step='miniprod', outdir=None):
    """
    ...
    if last_step == 'kingal':
        if outdir:
            copy_to_outdir(pythia_epio, outdir, log)
    ...
    if last_step == 'galeph':
        if outdir:
            copy_to_outdir(galeph_epio, outdir, log)
        write_file(status_file, 'DONE')
    ...
    if last_step == 'julia':
        if outdir:
            copy_to_outdir(julia_epio, outdir, log)
        write_file(status_file, 'DONE')
    ...
    if not dry_run:
        ...
        if outdir:
            copy_to_outdir(mini_epio, outdir, log)
